//===-- YCoreLowerThreadLocal - Lower thread local variables --------------===//
//
...
#define DEBUG_TYPE "ycore-lower-thread-local"

using namespace llvm;

static cl::opt<unsigned> MaxThreads(
  "ycore-max-threads", cl::Optional,
  cl::desc("Maximum number of threads (for emulation thread-local storage)"),
  cl::Hidden, cl::value_desc("number"), cl::init(8));

namespace {
  /// Lowers thread local variables on the YCore. Each thread local variable is
  ...
  struct YCoreLowerThreadLocal : public ModulePass {
    static char ID;

    YCoreLowerThreadLocal() : ModulePass(ID) {
      initializeYCoreLowerThreadLocalPass(*PassRegistry::getPassRegistry());
    }
...
char YCoreLowerThreadLocal::ID = 0;

INITIALIZE_PASS(YCoreLowerThreadLocal, "ycore-lower-thread-local",
                "Lower thread local variables", false, false)

ModulePass *llvm::createYCoreLowerThreadLocalPass() {
  return new YCoreLowerThreadLocal();
}
...
bool YCoreLowerThreadLocal::lowerGlobal(GlobalVariable *GV) {
...
bool YCoreLowerThreadLocal::runOnModule(Module &M) {
